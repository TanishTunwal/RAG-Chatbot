...
def ingest_pdf(file_bytes: bytes, thread_id: str, filename: Optional[str] = None) -> dict:
    ...
    try:
        ...
        if not chunks:
            raise ValueError(
                f"No extractable text found in '{filename or temp_path}'. "
                "This usually means the PDF is scanned or image-only and has no real text layer. "
                "Try a different PDF, or add OCR support if you need to handle scanned documents."
            )
